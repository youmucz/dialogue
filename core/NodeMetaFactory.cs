...
public class NodeMetaFactory
{
	...
	public static void Initialize()
	{
		...
		{
			if (!type.IsSubclassOf(typeof(DialogueNodeBase))) continue;
	...
	public static DialogueNodeBase GetNodeMeta(string nodeType, DialogueGraphResource resource, Godot.Collections.Dictionary data)
	{
		if (NodeMetaTypes.TryGetValue(nodeType, out var type))
		{
			var instance = (DialogueNodeBase)Activator.CreateInstance(type, new object[] { resource, data });
			return instance;
		}
	...
	public static DialogueNodeBase GetNodeMeta(string nodeType, DialogueGraphNode node, DialogueGraphResource resource, Godot.Collections.Dictionary data)
	{
		if (NodeMetaTypes.TryGetValue(nodeType, out var type))
		{
			var instance = (DialogueNodeBase)Activator.CreateInstance(type, new object[] { resource, node, data });
			return instance;
		}
