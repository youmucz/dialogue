...
public partial class VariableWindow : Control
{
    ...
    private DialogueGraphResource _dialogueGraphResource;
    ...
    private void LocalVariableTreeOnItemEdited()
    {
	    ...
	    _dialogueGraphResource.Variables[item.GetText(0)]["Variable"] = item.GetText(0);
	    _dialogueGraphResource.Variables[item.GetText(0)]["Value"] = item.GetText(1);
    }
    ...
    {
	    if (_dialogueGraphResource == null) return;
    ...
    private void SaveLocalVariable(Dictionary data)
    {
	    ...
	    TreeItem item;
	    if (_dialogueGraphResource.Variables.ContainsKey(variable))
	    {
		...
		_dialogueGraphResource.Variables[variable] = data;
    }
    ...
    private void RemoveLocalVariable()
    {
	    ...
	    if (_dialogueGraphResource.Variables.ContainsKey(item.GetText(0)))
	    {
		    _dialogueGraphResource.Variables.Remove(item.GetText(0));
	    }
    ...
    private void ResettingLocalVariable(TreeItem item=null)
    {
	    ...
	    if (_dialogueGraphResource.Variables.TryGetValue(item.GetText(0), out var data))
	    {
    ...
    private void SaveGlobalVariable(Dictionary data)
    {
	    ...
	    // TODO: 后续要改成全局变量池
	    TreeItem item;
	    if (_dialogueGraphResource.GlobalVariables.ContainsKey(variable))
	    {
    ...
    private void ResettingGlobalVariable(TreeItem item=null)
    {
	    ...
	    // TODO: 后续应该编辑全局变量池
	    if (_dialogueGraphResource.GlobalVariables.TryGetValue(item.GetText(0), out var data))
	    {
    ...
    public void LoadVariables(DialogueGraphResource resource)
    {
	    ClearVariables(resource);
	    
	    _dialogueGraphResource = resource;

	    foreach (var pair in _dialogueGraphResource.Variables)
	    {
    ...
    /// </summary>
    /// <param name="resource"></param>
    public void ClearVariables(DialogueGraphResource resource)
    {
	    _dialogueGraphResource = null;
