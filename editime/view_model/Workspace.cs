...
{
	public event Action<DialogueGraphResource> EventTabClosed = delegate { };
	public event Action<DialogueGraphResource> EventTabSelected = delegate { };
	...
	{
		EventTabClosed?.Invoke(GetTabEditor((int)tab).DialogGraphResource);
	...
	private void OnTabSelected(long tab)
	{
		...
		EventTabSelected?.Invoke(GetCurrentEditor().DialogGraphResource);
	}
	...
	public void SetCurrentTab(int index)
	{
		...
		EventTabSelected?.Invoke(GetTabEditor(index).DialogGraphResource);
	}
	...
	public int GetTabEditor(string filepath)
	{
		for (var i = 0; i < _tabContainer.GetTabCount(); i++)
		{
			...
			if (filepath == editor.DialogGraphResource.Filepath) return i;
		}
