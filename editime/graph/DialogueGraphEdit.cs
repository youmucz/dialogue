...
public partial class DialogueGraphEdit : GraphEdit
{
	...
	public DialogueGraphResource DialogGraphResource { get; private set; } = new ();
	...
	/// <summary>
	/// Load deserialized <see cref="DialogGraphResource" /> from local tres file, and rebuild graph.
	/// </summary>
	/// <param name="data"></param>
	public void LoadData(DialogueGraphResource data)
	{
		DialogGraphResource = data;
		DialogGraphResource.Initialize();

		Name = data.Filename?.Split(".")[0];

		foreach (var kvp in DialogGraphResource.NodeMetaClasses)
		{
			var newNode = CreateNode<DialogueGraphNode>((DialogueNodeBase)kvp.Value);
		...
		Player.DialogueResource = DialogGraphResource;
	}
	
	/// <summary>
	/// Return serialized node-graph data, include graph global param.
	/// </summary>
	/// <returns><see cref="DialogueGraphResource" /></returns>
	public DialogueGraphResource DumpsData()
	{
		...
		DialogGraphResource.NodeData = data;
		
		DialogGraphResource.Connection = GetConnectionList();
		
		Player.DialogueResource = DialogGraphResource;
		
		return DialogGraphResource;
	}
	...
	/// <typeparam name="T"></typeparam>
	public T CreateNode<T>(DialogueNodeBase @base) where T : DialogueGraphNode
	{
